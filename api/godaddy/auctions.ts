/**
 * Vercel Serverless Function — GoDaddy Auctions Proxy
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { limit = 50, minPrice = 1, maxPrice = 50, tlds = 'com,net,org,io,ai' } = req.query

    const url = `https://api.godaddy.com/v1/aftermarket/auctions?limit=${limit}&minPrice=${minPrice}&maxPrice=${maxPrice}&tlds=${tlds}&status=OPEN`

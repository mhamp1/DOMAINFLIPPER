...
import { listingOrchestrator, type MultiChannelListingResult } from '@/lib/marketplace/listingOrchestrator'
  ...
  async run(intelligenceSource: () => Promise<Domain[]>): Promise<PipelineResult> {
    ...
    try {
      ...
      purchaseResults = await this.runStage('Purchase', async () => {
        const results: Array<{ domain: Domain; success: boolean; price: number }> = []
      ...
      listingResults = await this.runStage('Listing', async () => {
        const results: MultiChannelListingResult[] = []
